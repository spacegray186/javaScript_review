...
function validate() {
    ...
    if( !(uid.length>=8 && uid.length<=12) ) {
        alert("아이디는 8~12 글자 이내로 입력해주세요");
        document.getElementById("uid").focus();
        return false;
    } // if end


    // 2) 비밀번호 글자개수가 5~10 사이인지 확인하시오
    let upw=document.getElementById("upw").value;
    upw=upw.trim(); // 맨앞과 맨뒤 공백 제거
    if( !(upw.length>=5 && upw.length<=10) ) {
        alert("비밀번호는 5~10 글자 이내로 입력해주세요");
        document.getElementById("upw").focus();
        return false;
    } // if end


    // 3) 주민번호 앞칸이 숫자형 글자 개수 6인지 확인하시오
    ...
    jumin2=jumin2.trim();
    if(jumin2.length!=7 || isNaN(jumin2)) { // 숫자기호인지?
